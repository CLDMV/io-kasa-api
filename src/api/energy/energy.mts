...
export async function getDayStats(target: DeviceTarget, year: number, month: number): Promise<Array<Record<string, number>>> {
...
export async function getMonthStats(target: DeviceTarget, year: number): Promise<Array<Record<string, number>>> {
	const response = await self.protocol.send(target, {
